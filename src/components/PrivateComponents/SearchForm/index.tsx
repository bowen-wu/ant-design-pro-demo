...
export default (props: PropsInterface) => {
  ...
  useEffect(() => {
    ...
    searchList.map((searchItem: SearchItemInterface) => Object.assign(searchInfoCopy, searchItem.type === 'rangePicker' ? {gmtCreateBegin: '', gmtCreateEnd: ''} : {[`${searchItem.key}`]: ''}));
  ...
  const searchTypeEle = (searchItem: SearchItemInterface) => {
    switch (searchItem.type) {
      case 'input':
        return (<Input placeholder={searchItem.placeholder || '请输入'} onChange={e => handleChange(e.target.value, searchItem.key)} />);
      case 'select': {
        const optionList = searchItem.optionList instanceof Array ? searchItem.optionList : [];
        const defaultArray = optionList.filter((option: SearchItemOptionInterface) => option.isDefault && option);
        ...
        return (
          ...
          >
            {optionList.map(option => <Option key={option.value}>{option.label}</Option>)}
          </Select>
        );
      }
      case 'rangePicker': {
        const defaultValue: any = (() => {
          if (searchItem.default) {
            if (searchItem.default instanceof Array && searchItem.default.length === 2) {
              return [moment(searchItem.default[0], rangePickerDateFormat), moment(searchItem.default[1], rangePickerDateFormat)];
            }
        ...
        return (<RangePicker defaultValue={defaultValue} placeholder={[placeholder, placeholder]} onChange={(date, dateStringArr) => updateSearchInfo({gmtCreateBegin: dateStringArr[0], gmtCreateEnd: dateStringArr[1]})} />);
      }
      case 'monthPicker':
        return (<MonthPicker defaultValue={moment(searchItem.default, monthPickerDateFormat) || ''} disabledDate={searchItem.disabledDate} placeholder={searchItem.placeholder || '请选择'} />)
      default:
        return null;
    }
  };
